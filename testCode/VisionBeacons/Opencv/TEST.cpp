...
int main(){
  ...
  while(1){
    ...
    cv::cvtColor(cameraFeed, cameraFeed, cv::COLOR_RGB2HSV);
    cv::inRange(cameraFeed, minHSV, maxHSV, out);
    cv::findContours(cameraFeed, cameraFeed, hierarchy, 0, cv::CHAIN_APPROX_SIMPLE)
    imshow("TESTing",cameraFeed);
  }
}
